...
DEFAULT_OLLAMA_HOST = os.getenv(
    "OLLAMA_HOST", "http://local_ip"
)  # Default to your powerful PC, fallback to localhost if needed
...
class DirectFirecrawlAgent:
    ...
    def __init__(
        ...
        self.agent = Agent(
            model=Ollama(id=model_id, host=ollama_host or DEFAULT_OLLAMA_HOST),
            markdown=True,
    ...
    def find_properties_direct(
        self, city: str, state: str, user_criteria: dict, selected_websites: list
    ) -> dict:
        ...
        urls_to_search = [
            url for site, url in search_urls.items() if site in selected_websites
        ]
...
EXTRACT EVERY VISIBLE PROPERTY LISTING - DO NOT LIMIT TO JUST A FEW!
        ...
        try:
            ...
            if hasattr(raw_response, "success") and raw_response.success:
                ...
                print(
                    f"Response data keys: {list(raw_response.data.keys()) if hasattr(raw_response, 'data') else 'No data'}"
                )
...
class LocalWebScrapingAgent:
    """Agent with local web scraping for property search - no external APIs required"""

    def __init__(self, model_id: str = "gpt-oss:20b", ollama_host: str = None):
        self.agent = Agent(
            model=Ollama(id=model_id, host=ollama_host or DEFAULT_OLLAMA_HOST),
            markdown=True,
            description="I am a real estate expert who helps find and analyze properties using local web scraping.",
        )
    ...
    def search_properties_google(
        self, city: str, state: str, user_criteria: dict
    ) -> List[str]:
        """Use Google search to find property listing URLs"""
        try:
            # Create search queries for different property sites
            budget_range = user_criteria.get("budget_range", "")
            property_type = user_criteria.get("property_type", "")

            search_queries = [
                f"{city} {state} homes for sale {property_type} {budget_range} site:zillow.com",
                f"{city} {state} real estate {property_type} site:realtor.com",
                f"{city} {state} houses for sale {property_type} site:trulia.com",
                f"{city} {state} property listings {property_type} site:homes.com",
            ]

            all_urls = []
            for query in search_queries:
                try:
                    # Limit to 5 results per site to avoid overwhelming
    ...
    def scrape_property_page(self, url: str) -> dict:
        """Scrape individual property page for details"""
        try:
            ...
            property_data = {
                ...
                "bedrooms": "Not found",
                "bathrooms": "Not found",
                "square_feet": "Not found",
                "property_type": "Not found",
                "description": "Not found",
            }

            # Extract based on common patterns across real estate sites
            # Price extraction
            price_selectors = [
                'span[data-testid="price"]',  # Zillow
                ".ldp-header-price",  # Realtor.com
                'div[data-testid="on-market-price-details"]',  # Zillow new
                ".price-section-price",  # Generic
                ".home-summary-price",  # Generic
            ]
            ...
            # Address extraction
            address_selectors = [
                'h1[data-testid="address"]',  # Zillow
                ".street-address",  # Realtor.com
                ".property-address",  # Generic
                ".listing-address",  # Generic
            ]

            for selector in address_selectors:
                addr_elem = soup.select_one(selector)
                if addr_elem:
                    property_data["address"] = addr_elem.get_text(strip=True)
                    break

            # Bedrooms/Bathrooms extraction
            bed_bath_selectors = [
                'span[data-testid="bed-bath-sqft-facts"]',  # Zillow
                ".property-beds-baths-sqft",  # Generic
                ".ldp-detail-facts",  # Realtor.com
            ]

            for selector in bed_bath_selectors:
                facts_elem = soup.select_one(selector)
                if facts_elem:
                    facts_text = facts_elem.get_text(strip=True)
                    # Extract numbers before "bed", "ba", "bath"
                    bed_match = re.search(r"(\d+)\s*bed", facts_text, re.I)
                    bath_match = re.search(r"(\d+(?:\.\d+)?)\s*ba", facts_text, re.I)
                    sqft_match = re.search(r"([\d,]+)\s*sqft", facts_text, re.I)
            ...
            type_selectors = [".property-type", ".home-type", ".listing-type"]
            ...
            return {
                ...
                "address": "Scraping failed",
                "price": "Not available",
            }

    def find_properties_local(self, city: str, state: str, user_criteria: dict) -> dict:
        """Local property search using Google search + web scraping"""
        try:
            print(f"Starting local property search for {city}, {state}")

            # Step 1: Find property URLs using Google search
            property_urls = self.search_properties_google(city, state, user_criteria)

            if not property_urls:
                return {
                    "error": "No property URLs found via Google search. This could be due to rate limiting or no results.",
                    "suggestion": "Try broadening your search criteria or using the Firecrawl option.",
                }

            print(f"Found {len(property_urls)} property URLs to scrape")

            # Step 2: Scrape each property URL
            properties = []
            max_properties = min(
                10, len(property_urls)
            )  # Limit to 10 to avoid overwhelming

            for i, url in enumerate(property_urls[:max_properties]):
                print(f"Scraping property {i + 1}/{max_properties}: {url}")
                property_data = self.scrape_property_page(url)

                # Only add properties with valid data
                if (
                    property_data.get("address") != "Scraping failed"
                    and property_data.get("price") != "Not available"
                ):
            ...
            if not properties:
                return {
                    "error": "No properties could be successfully scraped. Websites may have changed structure or are blocking requests.",
                    "suggestion": "Try using the Firecrawl option for more reliable extraction.",
                }
            ...
            return {
                ...
                "properties": properties,
                "total_count": len(properties),
                "source_websites": ["Local Web Scraping"],
                "method": "Google Search + BeautifulSoup",
            }

        except Exception as e:
            return {"error": f"Local scraping failed: {str(e)}"}


def create_sequential_agents(llm_host, user_criteria):
    """Create agents for sequential manual execution"""

    llm = Ollama(id="gpt-oss:20b", host=llm_host)
...
def run_sequential_analysis(
    ...
    property_search_agent, market_analysis_agent, property_valuation_agent = (
        create_sequential_agents(ollama_host, user_criteria)
    )

    # Step 1: Property Search - Choose method based on user preference
    update_callback(
        0.2,
        "Searching properties...",
        f"🔍 Property Search Agent: Finding properties using {search_method}...",
    )

    if search_method == "Local Web Scraping":
        # Use local web scraping method
        local_agent = LocalWebScrapingAgent(
            model_id="gpt-oss:20b", ollama_host=ollama_host
        )
        properties_data = local_agent.find_properties_local(
            city=city,
            state=state,
            user_criteria=user_criteria,
        )
    else:
        # Use Firecrawl method (default)
    ...
    update_callback(
        ...
        "💰 Property Valuation Agent: Evaluating properties...",
    )
    ...
    **Property [NUMBER]: [ADDRESS]**
    ...
    update_callback(
        0.95, "Synthesizing results...", "🤖 Synthesizing final recommendations..."
    )
...
"""
    ...
    all_text = (
        f"{json.dumps(properties, indent=2)} {market_analysis} {property_valuations}"
    )
    urls = re.findall(
...
def display_properties_professionally(
    properties, market_analysis, property_valuations, total_properties
):
    ...
    with col2:
        ...
        for p in properties:
            price_str = (
                p.get("price", "") if isinstance(p, dict) else getattr(p, "price", "")
            )
    ...
    with col3:
        ...
        for p in properties:
            t = (
                p.get("property_type", "Unknown")
                if isinstance(p, dict)
                else getattr(p, "property_type", "Unknown")
            )
        ...
        for i, prop in enumerate(properties, 1):
            ...
            with st.container():
                ...
                with col1:
                    ...
                    st.markdown(
                        f"**Beds/Baths:** {data['bedrooms']}/{data['bathrooms']}"
                    )
                ...
                with col2:
                    with st.expander("💰 Investment Analysis"):
                        ...
                        if property_valuation:
                            ...
                            st.info(
                                "Investment analysis not available for this property"
                            )
                with col3:
...
def main():
    ...
    st.caption(
        "Find Your Dream Home with Local Ollama AI Agents - Now with API-Free Option!"
    )
    ...
    with st.sidebar:
        ...
        with st.expander("🔑 API Keys", expanded=True):
            ...
            ollama_host = st.text_input(
                ...
                help="URL of your Ollama server (e.g., http://192.168.1.100:11434 for remote PC)",
                placeholder="http://localhost:11434",
            )

            # Test connection button
            if st.button(
                "🔍 Test Ollama Connection", help="Test if Ollama is accessible"
            ):
                with st.spinner("Testing connection..."):
                    try:
                        st.info(f"🔗 Testing connection to: {ollama_host}")
                        ...
                        if response.status_code == 200:
                            ...
                            if models:
                                ...
                                if any("gpt-oss" in name for name in model_names):
                                    st.success("🎯 Found gpt-oss model!")
                                else:
                                    st.warning("⚠️ gpt-oss:20b model not found. Run: `ollama pull gpt-oss:20b`")
                            else:
                                st.warning("⚠️ No models found. Run: `ollama pull gpt-oss:20b`")
                        else:
                            st.error(f"❌ Connection failed: HTTP {response.status_code}")
                            st.error(f"Response: {response.text}")
                            
                    except requests.exceptions.ConnectTimeout:
                        st.error("❌ Connection timeout - Ollama server not responding")
                        if "localhost" not in ollama_host:
                            st.error("🔍 **Remote PC Issues:**")
                            st.error("• Check if Ollama is running on the remote PC")
                            st.error("• Verify the IP address is correct")
                            st.error("• Ensure firewall allows port 11434")
                        else:
                            st.error("� **Local Issues:**")
                            st.error("• Run: `ollama serve` in terminal")
                            st.error("• Check if Ollama is installed")
                            
                    except requests.exceptions.ConnectionError as e:
                        st.error(f"❌ Connection failed: {str(e)}")
                        if "localhost" not in ollama_host:
                            st.error("🔍 **Remote PC Setup Required:**")
                            st.code("""# On my powerful PC:
# Windows:
...
ollama serve""", language="bash")
                            st.error("• Then restart Ollama service")
                            st.error("• Check Windows Firewall allows port 11434")
                        else:
                            st.error("• Make sure Ollama is installed and running")
                            
                    except Exception as e:
                        st.error(f"❌ Unexpected error: {str(e)}")
                        st.error(f"Error type: {type(e).__name__}")

            st.info("🤖 Using Ollama model: gpt-oss:20b")
            
            # Show setup instructions based on host
            if "localhost" not in ollama_host and "127.0.0.1" not in ollama_host:
                st.markdown("**Remote PC Setup Required:**")
                remote_ip = ollama_host.split("://")[1].split(":")[0]
                st.code(f"""# On your powerful PC ({remote_ip}):
...
ollama serve""", language="bash")
                st.warning("⚠️ Don't forget to allow port 11434 in Windows Firewall!")
            else:
                st.markdown("**Local Setup:**")
                st.code("ollama serve", language="bash")
                st.markdown("Or run: `ollama run gpt-oss:20b`")

        # Search Method Selection
        with st.expander("🔍 Search Method", expanded=True):
            search_method = st.radio(
                "Choose your property search method:",
                ["Firecrawl API", "Local Web Scraping"],
                help="Firecrawl requires API key but is more reliable. Local scraping is free but may be less reliable.",
                index=1,  # Default to Local Web Scraping
            )

            if search_method == "Firecrawl API":
                st.info("📡 Uses Firecrawl API for reliable web extraction")
                if not firecrawl_key:
                    st.warning("⚠️ Firecrawl API key required for this method")
            else:
                st.info("🆓 Uses local web scraping (Google Search + BeautifulSoup)")
                st.success("✅ No API keys required!")

        # Website selection
        with st.expander(
            "🌐 Search Sources", expanded=search_method == "Firecrawl API"
        ):
            if search_method == "Firecrawl API":
                ...
                st.markdown("**Local scraping searches across:**")
                st.markdown("• Zillow")
                st.markdown("• Realtor.com")
                st.markdown("• Trulia")
                st.markdown("• Homes.com")
                st.markdown("*Sources are automatically selected via Google search*")
                selected_websites = ["Auto-selected"]  # Dummy value for compatibility

        # How it works
        with st.expander("🤖 How It Works", expanded=False):
            if search_method == "Firecrawl API":
                st.markdown("**🔍 Property Search Agent**")
                st.markdown("Uses Firecrawl API for reliable web extraction")
            else:
                st.markdown("**🔍 Property Search Agent**")
                st.markdown("Uses Google Search + BeautifulSoup for local scraping")
    ...
    st.info(
        "Please provide the location, budget, and property details to help us find your ideal home."
    )
    ...
    if submitted:
        # Validate all required inputs based on search method
        missing_items = []
        if search_method == "Firecrawl API" and not firecrawl_key:
            missing_items.append("Firecrawl API Key")
        if not city:
            missing_items.append("City")
        if search_method == "Firecrawl API" and not selected_websites:
        ...
        try:
            ...
            update_progress(
                0.1, "Initializing...", "Starting sequential property analysis"
            )
            ...
            try:
                ...
                if test_response.status_code != 200:
                    raise Exception(f"Ollama server returned status {test_response.status_code}")
            except Exception as ollama_error:
                st.error("❌ **Ollama Connection Failed**")
                st.error(f"Cannot connect to Ollama at: {ollama_host}")
                st.error(f"Error: {str(ollama_error)}")
                
                st.markdown("### 🔧 **Quick Fix Solutions:**")
                
                if "localhost" in ollama_host or "127.0.0.1" in ollama_host:
                    st.markdown("**For Local Ollama:**")
                    st.code("""# Open terminal and run:
ollama serve

# Or in another terminal:
ollama run gpt-oss:20b""", language="bash")
                else:
                    remote_ip = ollama_host.split("://")[1].split(":")[0]
                    st.markdown(f"**For Remote Ollama ({remote_ip}):**")
                    st.code(f"""# On your powerful PC ({remote_ip}), run:
# Windows:
set OLLAMA_HOST=0.0.0.0:11434
ollama serve
...
# Make sure Windows Firewall allows port 11434""", language="bash")
                
                st.markdown("**Alternative: Use Local Ollama**")
                st.info("💡 Change the Ollama Host URL to `http://localhost:11434` and run Ollama locally")
                return
            ...
            download_content = (
                final_result["markdown_synthesis"]
                if isinstance(final_result, dict)
                else final_result
            )
